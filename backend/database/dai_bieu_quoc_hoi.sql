-- TRUNCATE xxxx RESTART IDENTITY CASCADE;

DROP TABLE FEEDBACK;
DROP TABLE RECOMMENDATIONS;
DROP TABLE ACCOUNTS;
DROP TABLE AGENCY;
DROP TABLE ROLES;
-- DROP TABLE LOCATIONS;

CREATE TABLE ACCOUNTS ( 
	ACCOUNTID SERIAL PRIMARY KEY,
	ACCOUNTNAME CHARACTER VARYING(50) NOT NULL UNIQUE,
	PASSWORD CHARACTER VARYING(255) NOT NULL,
	FULLNAME CHARACTER VARYING(50),
	DATEOFBIRTH TIMESTAMP,
	AVATAR TEXT, 
	POSITION CHARACTER VARYING(255), 
	CANDIDATEPLACE CHARACTER VARYING(255),
	STATUS CHARACTER VARYING(50),
	DATE_CREATED TIMESTAMP,
	UPDATED_DATE TIMESTAMP,
	AGENCYID SERIAL NOT NULL,
	ROLEID  SERIAL NOT NULL
);

CREATE TABLE AGENCY (
	AGENCYID SERIAL PRIMARY KEY,
	AGENCYNAME CHARACTER VARYING(255) NOT NULL UNIQUE,
	NOTE TEXT, 
	STATUS CHARACTER VARYING(50),
	DATE_CREATED TIMESTAMP,
	UPDATED_DATE TIMESTAMP
);

CREATE TABLE FEEDBACK (
	FEEDBACKID SERIAL PRIMARY KEY,
	TITLE CHARACTER VARYING(255),
	CONTENTS TEXT,
	FILES TEXT,
	STATUS CHARACTER VARYING(50),
	DATE_CREATED TIMESTAMP,
	UPDATED_DATE TIMESTAMP,
	ACCOUNTID SERIAL NOT NULL,
	RECOMMENDATIONSID SERIAL NOT NULL
);

TRUNCATE RECOMMENDATIONS RESTART IDENTITY CASCADE;
CREATE TABLE RECOMMENDATIONS (
	RECOMMENDATIONSID SERIAL PRIMARY KEY,
	OBJECT CHARACTER VARYING(50),
	FULLNAME CHARACTER VARYING(50),
	ADDRESS CHARACTER VARYING(255),
	PHONENUMBER CHARACTER VARYING(50),
	EMAIL CHARACTER VARYING(50),
	TITLE CHARACTER VARYING(50),
	CONTENTS TEXT, 
	FILES TEXT, 
	STATUS CHARACTER VARYING(50),
	FIELDS CHARACTER VARYING(255),
	REPORTINGDEADLINE TIMESTAMP, 
	COMMENTTYPE CHARACTER VARYING(50),
	DATE_CREATED TIMESTAMP,
	UPDATED_DATE TIMESTAMP,
	AGENCYID SERIAL NOT NULL
);

CREATE TABLE ROLES (
	ROLEID SERIAL PRIMARY KEY,
	ROLENAME CHARACTER VARYING(50) NOT NULL UNIQUE,
	KEYNAME CHARACTER VARYING(50) NOT NULL UNIQUE
);

-- CREATE TABLE LOCATIONS (
-- 	LOCATIONSID SERIAL PRIMARY KEY,
-- 	NAME CHARACTER VARYING(255),
-- 	PARENTID SERIAL,
-- 	DATE_CREATED TIMESTAMP,
-- 	UPDATED_DATE TIMESTAMP
-- );

INSERT INTO AGENCY(agencyname, status) VALUES
('Bộ tư pháp','Hoạt động'),
('Bộ công an','Hoạt động'),
('Bộ giáo dục và đào tạo','Hoạt động')

INSERT INTO roles(rolename, keyname) VALUES
('Quản trị viên','ADMIN'),
('Người dùng','USER');

INSERT INTO accounts(accountname, password, fullname, status, agencyid, roleid) VALUES
('admin','$2a$10$AS/Cr2jusR3p7ZwVbKixKega9/LYDDf9wi0T8ioXSHRSdjHpT0IQO','Quản trị viên','ACTIVE','1','1'),
('acc01','$2a$10$AS/Cr2jusR3p7ZwVbKixKega9/LYDDf9wi0T8ioXSHRSdjHpT0IQO','Nguyễn Văn A','ACTIVE','2','1')